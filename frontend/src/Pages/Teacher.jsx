...
const teacherData = [
    { id: 'T001', name: 'Dr. Evelyn Sharp', email: 'evelyn@uni.edu', subjects: ['Calculus I', 'Data Structures'], sectionAssignments: ['MATH-101', 'CS-205'] },
    { id: 'T002', name: 'Ms. Chloe Bennet', email: 'chloe@uni.edu', subjects: ['Digital Marketing', 'Physics Lab'], sectionAssignments: ['MKT-301', 'PHY-102'] },
    { id: 'T003', name: 'Prof. Mark Webber', email: 'mark@uni.edu', subjects: ['Econometrics'], sectionAssignments: ['ECO-401'] },
];
...
const announcements = [
    { id: 1, date: '2025-11-28', content: 'System maintenance scheduled for Dec 5th, 1 AM - 3 AM. Submissions will be blocked.', userType: 'All' },
    { id: 2, date: '2025-11-20', content: 'New plagiarism detection model activated. Please review the updated policy.', userType: 'Teacher, Admin' },
];
...
const Card = ({ title, value, icon: Icon, colorClass = 'text-indigo-400', children }) => (
...
const DetailsModal = ({ student, onClose, userRole, onSubmitFeedback }) => {
    ...
    const aiReport = {
        ...
        similarityToOthers: student.plagiarismPercent > 10 ? 'High similarity detected with 3 other submissions (needs review).' : 'Low similarity detected.',
        aiScoreSuggestion: student.score || 80, // Example AI score suggestion
        aiDeductions: student.plagiarismPercent > 10 ? ['Potential plagiarism (-20 marks)', 'Lack of original analysis'] : ['Minor grammatical issues'],
    };
            ...
            <div className="relative w-full max-w-4xl mx-auto bg-gray-900 rounded-xl shadow-2xl transform transition-all duration-300 scale-100 border border-gray-700">
                ...
                <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <h4 className="text-lg font-semibold text-white border-l-4 border-indigo-500 pl-3">Submission Details</h4>
                        <div className="grid grid-cols-2 gap-4 text-sm bg-gray-800 p-4 rounded-lg">
                            ...
                            <InfoItem label="Current Status" value={student.status} icon={List} color={student.status === 'Completed' ? 'text-green-400' : 'text-yellow-400'} />
                        ...
                        <section className="mt-6">
                            ...
                            <div className="bg-gray-800 p-4 rounded-lg space-y-3 mt-2 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className={`font-bold ${aiReport.plagiarism > 15 ? 'text-red-400' : 'text-green-400'}`}>{aiReport.plagiarism}%</span>
                                </div>
                                <div className="text-gray-400">
                                    <span className="font-semibold text-white">Originality Check:</span> {aiReport.similarityToOthers}
                                </div>
                                <div className="text-gray-400">
                                    <span className="font-semibold text-white">Suggested Deductions:</span>
                                    <ul className="list-disc pl-5 mt-1 text-xs">
                                        {aiReport.aiDeductions.map((d, i) => <li key={i}>{d}</li>)}
                                    </ul>
                                </div>
                            </div>
                        </section>
                    ...
                    {(userRole === 'Teacher' || userRole === 'Admin') && (
                        <div className="lg:col-span-1 space-y-6 bg-gray-800 p-4 rounded-xl border border-gray-700 h-full">
                            <h4 className="text-lg font-semibold text-white border-b border-gray-700 pb-2">Final Review & Grading</h4>
                            
                            {/* Final Score Input */}
                            <div>
                                <label htmlFor="finalScore" className="block text-sm font-medium text-gray-400 mb-1">Final Score ({student.maxScore} Max)</label>
                                <input
                                    id="finalScore"
                                    type="number"
                            ...
                            {/* Status Override */}
                            <div>
                                <label htmlFor="statusOverride" className="block text-sm font-medium text-gray-400 mb-1">Status Override (e.g., Fail, Re-submission)</label>
                                <select
                                    id="statusOverride"
                                    ...
                                    className="w-full rounded-lg border border-gray-700 bg-gray-900 px-4 py-2 text-white outline-none focus:border-indigo-500"
                                >
                                    {['Completed', 'In Review', 'Fail', 'Pass', 'Requires Resubmission'].map(s => (
                            ...
                            <div>
                                <label htmlFor="teacherNotes" className="block text-sm font-medium text-gray-400 mb-1">Teacher Review Message (for student)</label>
                                <textarea
                                    id="teacherNotes"
                                    value={teacherNotes}
...
// 3. Teacher Dashboard
const TeacherDashboard = ({ assignments, teachers, onDetailsClick, onSubmitFeedback }) => {
    ...
    return (
        <div className="p-4 md:p-8 lg:p-10 max-w-7xl mx-auto space-y-8">
            <h2 className="text-3xl font-bold text-white">Teacher Dashboard: {currentTeacher.name}</h2>
            <p className="text-gray-400">Assigned Sections: {currentTeacher.sectionAssignments.join(', ')}</p>
            ...
            <h3 className="text-2xl font-semibold text-white pt-4 border-t border-gray-800">Assignments to Review ({sortedAssignments.length})</h3>
            <div className="space-y-4">
                {sortedAssignments.map((student) => {
                    ...
                    return (
                        <div 
                            key={student.id + student.assignmentId} 
                            className={`flex justify-between items-center p-4 sm:p-6 rounded-xl border ${isPending ? 'border-yellow-600/50 bg-yellow-900/10' : 'border-gray-800 bg-gray-900/50'} shadow-lg transition-all duration-200 ease-in-out hover:bg-gray-800/80`}
                        >
                            ...
                            <div className="flex items-center flex-shrink-0 ml-4 space-x-6">
                                ...
                                <div className="flex flex-col items-end mr-4">
                                    <StatusBadge status={student.status} isHighPlag={isHighPlagiarism} />
                                    <span className={`text-xs mt-1 ${isHighPlagiarism ? 'text-red-400' : 'text-gray-400'}`}>Plag: {student.plagiarismPercent}%</span>
                                </div>

                                {/* Review Button */}
                                <button
                                    onClick={() => onDetailsClick(student)}
                                    className={`flex items-center justify-center p-3 rounded-full ${isPending ? 'bg-red-600' : 'bg-indigo-600'} text-white shadow-md hover:bg-indigo-700 transition-colors`}
...
const AdminDashboard = ({ data, teachers, students, onDetailsClick }) => {
    ...
    const teacherUserList = teachers.map(t => ({ id: t.id, name: t.name, role: 'Teacher', email: t.email }));
    const studentUserList = students.map(s => ({ id: s.id, name: s.name, role: 'Student', email: `${s.name.toLowerCase().replace(' ', '.')}@uni.edu` }));
    ...
    return (
        <div className="p-4 md:p-8 lg:p-10 max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <Card title="Total Students" value={data.totalStudents} icon={Users} colorClass="text-indigo-400" />
                <Card title="Total Teachers" value={data.totalTeachers} icon={User} colorClass="text-green-400" />
                <Card title="Pending Enrollments" value={data.pendingEnrollments} icon={Clock} colorClass="text-yellow-400" />
                <Card title="System Status" value={data.systemStatus} icon={Settings} colorClass="text-emerald-400" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
                ...
                <div className="bg-gray-900 p-6 rounded-xl border border-gray-800 space-y-4">
                    <h4 className="text-xl font-semibold text-white flex items-center"><Bell className="h-5 w-5 mr-2 text-yellow-400" /> Make Global Announcement</h4>
                    <textarea
                    ...
                    <h5 className="text-sm font-medium text-gray-400 pt-2">Recent Announcements:</h5>
                    <ul className="text-xs space-y-1 text-gray-400">
                        {announcements.map(a => (
                            <li key={a.id} className="p-2 bg-gray-800 rounded-md border-l-4 border-yellow-500/50">
                                <span className="font-medium text-white">[{a.date}]</span> {a.content}
                            </li>
                        ))}
                    </ul>
                </div>
                ...
                <div className="bg-gray-900 p-6 rounded-xl border border-gray-800 space-y-4">
                    <h4 className="text-xl font-semibold text-white flex items-center"><Lock className="h-5 w-5 mr-2 text-red-400" /> Block/Unblock User</h4>
                    <input
                        type="text"
                        value={userToBlock}
                    ...
                    <h4 className="text-xl font-semibold text-white pt-4 flex items-center"><Layers className="h-5 w-5 mr-2 text-indigo-400" /> Teacher Assignment Allocation</h4>
                    <p className="text-sm text-gray-400">Current allocation: T001 gets MATH-101 & CS-205. This can be managed here.</p>
                </div>
            </div>

            {/* Registered Users List */}
            <h3 className="text-2xl font-semibold text-white pt-4 border-t border-gray-800">Registered Users ({allUsers.length})</h3>
                        ...
                        {allUsers.map((user) => (
                            <tr key={user.id} className="hover:bg-gray-800/80 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${user.role === 'Teacher' ? 'bg-indigo-600/20 text-indigo-400' : 'bg-green-600/20 text-green-400'}`}>
                                        {user.role}
                                    </span>
                                </td>
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="flex h-20 items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4">
                        <span className="text-sm font-medium text-gray-400 hidden sm:block">Current Role:</span>
                        <select
                            value={currentUser}
...
const App = () => {
    ...
    const handleSubmitFeedback = useCallback((assignmentId, feedback) => {
        ...
        alert(`Feedback submitted and assignment ${assignmentId} finalized! Score: ${feedback.score}`);
    }, []);

    const renderDashboard = () => {
        switch (currentRole) {
            case 'Student':
                return <StudentDashboard assignments={assignments} onDetailsClick={setSelectedAssignment} />;
            case 'Teacher':
                return <TeacherDashboard assignments={assignments} teachers={teacherData} onDetailsClick={setSelectedAssignment} onSubmitFeedback={handleSubmitFeedback} />;
            case 'Admin':
                return <AdminDashboard data={adminData} teachers={teacherData} students={assignments} onDetailsClick={setSelectedAssignment} />;
            default:
