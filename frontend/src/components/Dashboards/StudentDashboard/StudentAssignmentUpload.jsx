...
import { 
    ChevronLeft, Calendar, Clock, CheckCircle, Upload, FileText, AlertCircle, X, Eye, Download
} from 'lucide-react';
...
const assignmentDb = {
    '1': {
        ...
        status: 'Submitted', 
        maxScore: 100,
        instructions: "Basic concepts of OS.",
        submittedFile: "Alex_Basics.pdf",
        submissionDate: "2025-10-14",
        score: 92,
        feedback: "Good work on the definitions."
    },
    '2': {
        id: '2',
        ...
        status: 'Pending', 
        maxScore: 100,
        instructions: "Please upload your solution in PDF format. Ensure all algorithms are explained clearly.",
        submittedFile: null,
        submissionDate: null,
        score: null,
        feedback: null
...
const StudentAssignmentUpload = () => {
    ...
    // Load specific assignment data or fallback
    const data = assignmentDb[id] || assignmentDb['2'];

    // State
    const [status, setStatus] = useState(data.status);
    const [file, setFile] = useState(null);
    const [uploadProgress, setUploadProgress] = useState(0);
    const [submittedFileName, setSubmittedFileName] = useState(data.submittedFile);
    const [submissionTime, setSubmissionTime] = useState(data.submissionDate);
    ...
    const handleViewFile = () => {
        if (file) {
            // If user just uploaded it, we have the file object in memory
            const fileURL = URL.createObjectURL(file);
            window.open(fileURL, '_blank');
        } else {
            // If it's a past submission (dummy data), we simulate viewing
            alert(`Opening ${submittedFileName}... (Backend required for real download)`);
        }
    };
    ...
    return (
        <div className="relative z-10 bg-gray-950 min-h-screen p-4 md:p-8 lg:p-10 font-sans text-white">
            ...
            {/* 1. Back Button Row - Pushed down to avoid header overlap */}
            ...
            <div className="max-w-5xl mx-auto mb-8 border-b border-gray-800 pb-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end">
                    <div>
                        ...
                        <p className="text-indigo-400 font-medium">{data.subject}</p>
                    </div>
                    
                    {/* Status Badge */}
                    <div className={`mt-4 md:mt-0 px-4 py-2 rounded-full text-sm font-bold border flex items-center ${
                        status === 'Submitted' 
                        ? 'bg-green-500/10 text-green-400 border-green-500/20' 
                        : 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20'
                    }`}>
                        {status === 'Submitted' ? <CheckCircle className="h-4 w-4 mr-2" /> : <Clock className="h-4 w-4 mr-2" />}
            ...
            <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className="bg-[#1a1f2e] p-4 rounded-xl border border-gray-800 flex items-center">
                    ...
                    <div>
                        <p className="text-xs text-gray-400 uppercase tracking-wider">Unlocked On</p>
                        <p className="font-semibold">{data.unlockDate}</p>
                    </div>
                </div>
                <div className="bg-[#1a1f2e] p-4 rounded-xl border border-gray-800 flex items-center">
                ...
                <div className="bg-[#1a1f2e] p-4 rounded-xl border border-gray-800 flex items-center">
                    ...
                    <div>
                        <p className="text-xs text-gray-400 uppercase tracking-wider">Max Marks</p>
                        <p className="font-semibold">{data.maxScore}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-[#1a1f2e] p-8 rounded-2xl border-2 border-dashed border-gray-700 hover:border-indigo-500/50 transition-colors text-center group relative">
                            {!file ? (
                                <>
                                    ...
                                    <h3 className="text-lg font-semibold text-white mb-2">Upload your work</h3>
                                    <p className="text-gray-400 text-sm mb-6">Drag & drop your PDF here or click to browse</p>
                                    <input 
                                        type="file" 
                                ...
                                <div className="text-left">
                                    <div className="flex items-center justify-between bg-gray-900 p-4 rounded-xl border border-gray-700 mb-4">
                                        ...
                                        <button onClick={handleRemoveFile} className="p-2 hover:bg-gray-800 rounded-full transition-colors">
                                    ...
                                    >
                                        {uploadProgress > 0 ? (uploadProgress === 100 ? 'Uploaded!' : 'Uploading...') : 'Submit Assignment'}
                                    </button>
                                </div>
                            )}
                        </div>
                    ...
                    {status === 'Submitted' && (
                        <div className="bg-green-500/5 p-6 rounded-2xl border border-green-500/20 text-center">
                            ...
                            <h3 className="text-xl font-bold text-white mb-2">Assignment Submitted!</h3>
                            ...
                            <div className="bg-[#1a1f2e] p-4 rounded-xl border border-gray-700 flex items-center justify-between text-left mb-4 max-w-lg mx-auto shadow-lg">
                                <div className="flex items-center overflow-hidden">
                                    ...
                                    <div className="min-w-0">
                                        <p className="text-sm font-medium text-white truncate max-w-[200px]">{submittedFileName || "submission.pdf"}</p>
                            ...
                            <div className="inline-flex items-center px-4 py-2 bg-gray-900 rounded-lg border border-gray-800 text-sm text-gray-400">
                                ...
                                Submitted on: <span className="text-white ml-2">{submissionTime || "2025-10-14"}</span>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="lg:col-span-1">
                    <div className="bg-[#1a1f2e] p-6 rounded-2xl border border-gray-800 h-fit sticky top-32">
                        <h4 className="text-lg font-bold text-white mb-4 border-b border-gray-800 pb-4">Evaluation</h4>
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <p className="text-3xl font-bold text-white tracking-tight">
                                    {data.score ? (
                                        <span className="text-green-400">{data.score} <span className="text-lg text-gray-500">/ {data.maxScore}</span></span>
                                    ) : (
                            ...
                            <div>
                                <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Teacher Feedback</p>
                                <div className="p-4 bg-gray-900/50 rounded-lg border border-gray-800 text-sm text-gray-400 italic min-h-[100px]">
                                    {data.feedback ? `"${data.feedback}"` : "No feedback available yet."}
                                </div>
                            </div>
                        </div>
                    </div>
